...
| damping                   | `number`  | 10      | How quickly a spring stops moving. Higher damping means the spring will come to rest faster.                               |
| duration                  | `number`  | 2000    | Length of the animation (in milliseconds).                                                                                 |
| dampingRatio              | `number`  | 0.5     | How damped the spring is. Value `1` means the spring is critically damped. Values `>= 1` means the spring won't overshoot. |
