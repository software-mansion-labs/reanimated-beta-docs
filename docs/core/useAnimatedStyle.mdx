...
- The callback passed to the `useAnimatedStyle` is first run on the [JavaScript thread](/docs/fundamentals/glossary#javascript-thread) and immediately after on the [UI thread](/docs/fundamentals/glossary#ui-thread). Some functions like `measure` and `scrollTo` have implementations only on the UI thread. To safely use them inside of `useAnimatedStyle` callback wrap your code with a check:

<Indent>

</Indent>

- Stay away from mutating shared values in `useAnimatedStyle`'s callback. In many cases this can cause an infinite loop.
