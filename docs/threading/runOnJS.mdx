...
A reference to the function you want to execute on the [JavaScript thread](/docs/fundamentals/glossary#javascript-thread) from the [UI thread](/docs/fundamentals/glossary#ui-thread). Arguments to your function have to be passed to the function returned from `runOnJS` i.e. `runOnJS(setValue)(10);`.
...
- Make sure you pass a reference to the function instead of executing it in place, e.g. ~~runOnJS(setValue(10))()`~~.
