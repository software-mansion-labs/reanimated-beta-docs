...
Most commonly used either with an `useEffect` to start an animation on component mount/unmount or with [`measure`](/docs/advanced/measure) and [`scrollTo`](/) functions which have implementations only on the UI thread.
...
- Make sure you pass a reference to the function instead of executing it in place, e.g. ~~runOnUI(setValue(10))()`~~.

- When implementing your animations you should first reach for more general solutions such as [`useDerivedValue`](/), [`useAnimatedReaction`](/docs/advanced/useAnimatedReaction) or running code in gesture callbacks and only use `runOnUI` after you've tried other methods.

- The callback passed as the argument is automatically [workletized](/docs/fundamentals/glossary#to-workletize) and ran on the [UI thread](/docs/fundamentals/glossary#ui-thread).
