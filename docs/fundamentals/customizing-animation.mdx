...
Previous sections not only taught you how to use shared values in practice but also you used `withSpring` and `withTiming` functions to create animations. We think now you're more than ready to dive deeper into customizing animations!

Reanimated comes with three built-in animation functions: [`withTiming`](/docs/animations/with-timing), [`withSpring`](/docs/animations/with-spring), and [`withDecay`](/docs/animations/with-decay). For now, let's focus on the first two, and we'll come back to `withDecay` in the [Handling interactions](/docs/fundamentals/handling-interactions) section.
...
Simple enough, the `duration` parameter defines how long in milliseconds the animation should take to reach the assigned `toValue`. The duration by default is set to `300`.

The `easing` parameter lets you fine-tune the animation over the specified time. For example, you can make the animation begin with fast acceleration and then slow it down towards the end, or start slowly then pick up some speed before slowing down again towards the end. This value defaults to `Easing.inOut(Easing.quad)`.
...
Reanimated comes with a handful of predefined easing functions. You can play around with them in the interactive playground below or check the full [`withTiming` API reference](/docs/animations/with-timing).
...
Damping describes how quickly the spring slows down. In the real world, you could think about the same spring bouncing in the air and underwater. For example, a spring in a vacuum would have zero friction and thus zero damping. Higher damping means the spring will come to rest faster.

Reanimated comes with a handful of other properties used to customize your spring animation. You can play around with them in our interactive playground or check the full [`withSpring` API reference](/docs/animations/with-spring).
...
In [the upcoming section](/docs/fundamentals/handling-interactions), you'll discover how to use animation modifiers such as `withSequence` and `withRepeat`. These modifiers we'll allow us to create more complex and engaging animations.
