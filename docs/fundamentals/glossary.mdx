...
Animated components are components which Reanimated can animate. Reanimated comes with a bunch of wrapped React Native built-ins such as `Animated.View`, `Animated.ScrollView`, or `Animated.FlatList`.
...
You can make your own React Native components animatable by wrapping them with [`createAnimatedComponent`](docs/core/createAnimatedComponent) function.
...
When used, the data stored in a shared value is automatically synchronized between the JS thread and the UI thread.
...
Reanimated comes with just a handful of built-in components like `Animated.View` or `Animated.ScrollView`. For components which aren't a part of Reanimated, to make their props animatable, you wrap them with [`createAnimatedComponent`](/docs/core/createAnimatedComponent):
