...
- Using `useAnimatedStyle` gives more control over the animation and allows to keep all the animation-related logic in one place.
- Difference between animating `props` and `styles` is that props are not passed to the `style` object, but rather as a separate props to the component.
