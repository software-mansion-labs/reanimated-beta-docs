...
As we learned in the [previous section](/docs/fundamentals/your-first-animation) we can animate styles by [passing Shared Values inline](/docs/fundamentals/glossary#animations-in-inline-styling) to the elements' `style` property:
...
If we would like to customize how our shared value changes based on some user input, (e.g. making it twice as fast or following some mathematical equation) we couldn't use inline styling.
...
You can see it action in the example below:

<InteractiveExample name="AnimatingStyles" component={<AnimatingStyles />} />

## Animating props

Most of the values that developers animate (`width`, `color`, `transform` etc.) are modified by passing them as an object to `style` property of an element. But that's not always the case.

Sometimes we'd like to also animate not just styles but also the props which are passed to the component.
...
Reanimated comes with just a handful built-in components like `Animated.View` or `Animated.ScrollView`. For components which aren't a part of Reanimated, in order to make their props animatable, we need to wrap them with `createAnimatedComponent`:
...
This approach works just fine but same as `useAnimatedStyle` for animating styles we can encapsulate the animation logic and gain access to `.value` property of a Shared Value by using `useAnimatedProps`.

So if we'd like to smoothly increases the radius of a circle by `10px` on each button press we could use `useAnimatedProps`:
...
In a function which `useAnimatedProps` takes as an argument we return an object with all the props we'd like to animate. Then we can pass the value which `useAnimatedProps` return to `animatedProps` props of an Animated component.

Check out the full example below:

<InteractiveExample name="AnimatingProps" component={<AnimatingProps />} />

## Summary

In this section, we went through the differences between animating styles and props and how use `useAnimatedStyle` and `useAnimatedProps` hooks. To sum up:

- Passing Shared Values to inline styles is a simple way of creating animations but it has some limitations.
- Difference between animating `props` and `styles` is that props are not passed to the `style` object, but rather as a separate props of the component.
...
- By using `useAnimatedStyle` and `useAnimatedProps`, you can keep all the animation-related logic in one place.
- You can make your own animatable components by wrapping them with `Animated.createAnimatedComponent`.

## What's next?

In [the next section](applying-modifiers), we'll learn more about animation functions and how to customize their behavior.
