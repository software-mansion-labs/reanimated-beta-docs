...
In short, the Reanimated babel plugin automatically converts special JavaScript functions (called [worklets](/worklets)) in a way that they could be passed and run on the UI thread.

To learn more about the plugin head onto to [Reanimated babel plugin](/docs/learn-more/reanimated-babel-plugin) section.
...
More advanced use cases such as running Reanimated with `webpack` or with `Next.js` are explained in a separate [Web Support](/web-support) guide.
